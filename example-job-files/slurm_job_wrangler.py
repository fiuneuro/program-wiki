...
def _get_parser():
    ...
    parser.add_argument(
        ...
        action='store',
        type=int,
        help='Maximum number of jobs to run at once. If set, you should submit '
...
def main(template_job_file, participant_label=None, tsv_file=None, txt_file=None,
         job_limit=None):
    """
    Submit jobs for multiple participants. Optimal for running preprocessing
    BIDS Apps in parallel instead of sequentially.
    ...
    participant_label : str or list or None, optional
        ...
        prefix can be removed). Mutually exclusive with "tsv_file" and "txt_file".
        One of "participant_label", "tsv_file", and "txt_file" must be provided.
    ...
    if (participant_label is not None) and isinstance(participant_label, list):
        ...
        raise Exception('One of "participant_label", "tsv_file", and '
    ...
    for subject in subjects:
        ...
        if job_limit is None:
            ...
            while True:
                ...
                squeue_cmd = 'squeue -u {} > {}'.format(username, job_tracker_file)
                run(squeue_cmd)
